...
try:
    ...
    print("⚠️ Lỗi khi tải model CF:", e)
    model_artist = pivot_artist = pivot_sparse = None

# -------------------------
# 3️⃣ HÀM FALLBACK CHO USER MỚI
# -------------------------
...
# -------------------------
# 4️⃣ HÀM GỢI Ý CHÍNH (THEO LOGIC CELL ③)
# -------------------------
def recommend_for_user(user_id, top_n=8, n_neighbors=60, months_back=3, verbose=False):
    ...
    (bản tương đương logic với Cell ③ - nhưng dùng MongoDB)
    """
    try:
        ...
        if not recs:
            if verbose: print(f"⚠️ Không tạo được gợi ý cho user {user_id}.")
            return get_popular_artists(top_n)
        ...
        # )
        #
        # # Fallback nếu thiếu nghệ sĩ mới
        ...
        # --- Phần 5: Chia nhóm Quen / Mới (SỬA LẠI HOÀN TOÀN) ---
        top_recent_set = set(recent_df["artistName"].value_counts().head(10).index.tolist())

        print(f"🔍 DEBUG: rec_df có {len(rec_df)} candidates trước khi filter")
        print(f"🔍 DEBUG: top_recent_set có {len(top_recent_set)} artists")

        # 🎯 SỬA: Lấy TOP top_n artists từ rec_df, KHÔNG phân chia familiar/new
        ...
        # 🎯 Nếu không đủ artists mới, điều chỉnh
